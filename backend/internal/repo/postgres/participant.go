...
func (r ParticipantRepo) UpdateParticipant(ctx context.Context, p entities.Participant) error {
	query := "UPDATE participants SET name = COALESCE(NULLIF($1, ''), name), team_id = COALESCE(NULLIF($2, 0), team_id), role = COALESCE(NULLIF($3, ''), role), birthdate = COALESCE(NULLIF($4, NULL), birthdate), birthplace = COALESCE(NULLIF($5, ''), birthplace) WHERE id = $6"
